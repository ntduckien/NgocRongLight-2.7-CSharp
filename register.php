...
if ($num_accounts >= $max_accounts_per_ip) {
   ...
   if ($_login == null && isset($_POST['username']) && isset($_POST['password'])) {
      ...
      if (empty($_POST['password'])) {
         ...
         $read = _select("*", 'user', "username='$username'");
         ...
         if (is_array($existing_account)) {
            ...
            $txt = _insert('user', 'username,password,ip_address', "'$username','$password','{$_SERVER['REMOTE_ADDR']}'");
            $kiemtra = _query($txt);
            if ($kiemtra) {
...
function count_accounts_by_ip($ip_address)
{
   global $conn;
   $count = 0;
   $result = _select("COUNT(*) as count", "user", "ip_address='$ip_address'");
