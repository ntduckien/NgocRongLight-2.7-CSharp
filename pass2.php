...
if (isset($_POST['password'])) {
   ...
   if ($old_pass != $_password) {
      ...
      $alert_messages[] = "<script type='text/javascript'>$(document).ready(function() { swal('Thất bại', 'Mật khẩu cấp 2 không trùng nhau!', 'error'); });</script>";
   } else {
      $stmt = $conn->prepare("UPDATE user SET mkc2 = ? WHERE username = ?");
      $stmt->bind_param("ss", $new_pass, $_username);
      if ($stmt->execute()) {
         if ($stmt->affected_rows > 0) {
            $alert_messages[] = "<script type='text/javascript'>$(document).ready(function() { swal('Thành công', 'Cập nhật mật khẩu cấp 2 thành công!', 'success'); });</script>";
         } else {
            ...
            <main>
               <div class="row">
                  <div class="col-md-9 pb-3 pt-2">
                     ...
                     <p class="text-muted">Mật khẩu cần có 6 ký tự trở lên, nên sử dụng cả ký tự, chữ cái và số
               ...
               <div class="password-change-messages">
                  <?php
                  foreach ($alert_messages as $message) {
                     echo $message;
                  }
                     ...
                     <form method="POST">
                        ...
                        <div class="mb-3">
                           ...
                           <input type="password" class="form-control" name="passwordcap2xacnhan"
                              ...
                              autocomplete="passwordcap2xacnhan">
                        </div>
